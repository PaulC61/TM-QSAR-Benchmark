...
def calc_regression_metrics(df, cycle_col, val_col, pred_col, thresh):
    ...
    metric_df = pd.DataFrame(metric_list, columns=["Split", "Model", "Fold"] + metric_ls)
    return metric_df
...
def rm_tukey_hsd(df, metric, group_col, alpha=0.05, sort = False, direction_dict=None):
    ...
    with warnings.catch_warnings():
        ...
        aov = pg.rm_anova(dv=metric, within=group_col, subject='Split', data=df, detailed=True)
...
def make_boxplots_parametric(df, metric_ls):
    ...
    for i, stat in enumerate(metric_ls):
        model = AnovaRM(data=df, depvar=stat, subject='Split', within=['method']).fit()
...
def make_boxplots_nonparametric(df, metric_ls):
    ...
    for i, stat in enumerate(metric_ls):
        friedman = pg.friedman(df, dv=stat, within="method", subject="Split")['p-unc'].values[0]
        ax = sns.boxplot(y=stat, x="method", hue="method", ax=axes[i], data=df, palette="Set2", legend=False)
...
def make_sign_plots_nonparametric(df, metric_ls):
    ...
    for i, stat in enumerate(metric_ls):
        pc = sp.posthoc_conover_friedman(df, y_col=stat, group_col="method", block_col="Split", p_adjust="holm",
                                         melted=True)
...
def make_critical_difference_diagrams(df, metric_ls):
    ...
    for i, stat in enumerate(metric_ls):
        avg_rank = df.groupby("Split")[stat].rank(pct=True).groupby(df.method).mean()
        pc = sp.posthoc_conover_friedman(df, y_col=stat, group_col="method", block_col="Split", p_adjust="holm",
                                         melted=True)
...
def make_normality_diagnostic(df, metric_ls):
    ...
    for metric in metric_ls:
        df_norm[metric] = df_norm[metric] - df_norm.groupby("Model")[metric].transform("mean")

    df_norm = df_norm.melt(id_vars=["Split", "Model", "Fold"],
...
def make_scatterplot(df, val_col, pred_col, thresh, cycle_col="Split", group_col="method"):
    """
...
def calc_classification_metrics(df_in, cycle_col, val_col, prob_col, pred_col):
    ...
    metric_df = pd.DataFrame(metric_list, columns=["Split", "Model", "Fold",
                                                    "roc_auc", "pr_auc", "mcc", "recall", "tnr"])
    return metric_df


def make_curve_plots(df):
    df_plot = df.query("Split == 0 and Group == 'scaffold'").copy()
