...
MACRO_OUT_FILENAME = "results/MACRO_TM_Benchmark_8"
MICRO_OUT_FILENAME = "results/MICRO_TM_Benchmark_8"
...
def benchmark_CLF_objective(trial, X_train_in, X_val_in, Y_train_in, Y_val_in, model_label, param_grid):
    ...
    if model_label == "RandomForest":
        ...
        clf_model = TMCoalescedClassifier(**clf_params)
    ...
    if model_label != "TsetlinMachine":
        ...
        for epoch in range(int(N_TM_EPOCHS)):
            ...
            _, Y_val_ccs = clf_model.predict(X_val_in, return_class_sums=True)
            ...
            for fold, [train_indx, test_indx] in enumerate(kf_5.split(dataset_df, groups=current_group)):
                ...
                for descriptor_indx in range(len(DESCRIPTOR_SET)):
                    ...
                    for model_indx in range(len(MODEL_LABELS)):
                        ...
                        if split > 3:
                            print(split, "CV Param Search Started")
                            cv_best_params = []
                            hp_search_time = 0
                            ...
                            for v_fold, [train_val_indx, val_indx] in enumerate(kf_2.split(train_df, groups=current_group[train_indx])):
                                ...
                                if task == "class":
                                    ...
                                    hp_search_start = time.time()
                                    study.optimize(
                                        lambda trial: benchmark_REG_objective(
                                            trial,
                                            X_train_in = X_train_val,
                                            X_val_in = X_val,
                                            Y_train_in = Y_train_val,
                        ...
                        #else:
                        #    print(split, "CV Param Search Skipped")
                        #    split_mean_best_params = dict()
                        #    for split_param in split_params[descriptor]:
                        #        isInt = split_params[split_param][descriptor][-1]
                        #        split_values = []
                        #        for split_n in range(N_PARAM_SPLITS):
                        #            split_best_params = split_params[split_n]
                        #            split_values.append(split_best_params[split_param])
                        #        if isInt:
                        #            split_mean_best_params[split_param] = int(statistics.mean(split_values))
                        #        else:
                        #            split_mean_best_params[split_param] = statistics.mean(split_values)
                        #        final_params = split_mean_best_params
                            ...
                            if task == "class":
                                if model_label != "TsetlinMachine":
                                    ...
                                    for epoch in range(N_TM_EPOCHS):
                                        ...
                                        _, Y_test_ccs = clf_model.predict(X_test, return_class_sums=True)
